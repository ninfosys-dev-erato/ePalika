...
function selectAdapter(): Promise<AuthAdapter> {
  if (!adapterPromise) {
    ...
    if (import.meta.env.VITE_AUTH_ADAPTER === 'federated') {
      adapterPromise = import('./adapters/federated').then(m => new m.FederatedKeycloakAdapter())
    } else {
      adapterPromise = import('./adapters/simulated').then(m => new m.SimulatedAdapter())
    }
  }
...
export async function onAuthEvent(event: 'token' | 'login' | 'logout' | 'error', cb: () => void) {
  const a = await selectAdapter()
