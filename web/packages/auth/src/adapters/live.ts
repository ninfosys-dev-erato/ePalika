...
export class LiveKeycloakAdapter implements AuthAdapter {
  ...
  async init(cfg: BaseAuthConfig): Promise<AuthInitResult> {
    if (!this.ensureSecureCrypto()) {
      console.warn(
        "⚠️ Web Crypto API not available/secure; Keycloak disabled."
      );
    ...
    try {
      const authenticated = await this.kc.init({
        ...
        silentCheckSsoRedirectUri:
          ...
          import.meta.env.VITE_SILENT_CHECK_SSO_URI,
      });
      ...
      if (this.refreshTimer == null) {
        this.refreshTimer = window.setInterval(async () => {
          try {
            const didRefresh = await this.kc?.updateToken(
  ...
  async logout(options?: { redirectUri?: string }) {
    ...
    await this.kc?.logout({
      redirectUri: options?.redirectUri ?? location.origin,
    });
    this.emit("logout");
  }
